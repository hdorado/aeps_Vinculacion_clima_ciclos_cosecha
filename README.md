...
El presente proyecto desarrollado en `Rproject` tiene el propósito de describir el proceso realizado para el procesamiento de variables climáticas en ciclos de cultivo, donde la información fuente son estaciones meteorológicas. Este es un paso importante en el análisis de agricultura específica por sitio, debido a que el clima desempeña un rol fundamental en la explicación de la variación en rendimiento. Los pasos descritos serán de utilidad cuando se tiene sitios de interés georreferenciados, cuya información está dada en un periodo de tiempo, y además se quiere evaluar la relación con estaciones meteorológicas dentro de la misma localidad. El archivo que se obtiene como resultado de este proceso contendrá un conjunto de variables climáticas (promedios, acumulados, conteos,…) asignados a cada ciclo de cultivo y que posteriormente se puede usar para clustering o modelos supervisados.
...
El archivo **Merge_Stations_Funs.R**, contiene funciones programadas que son utilizadas en el script *Distances_Process*
